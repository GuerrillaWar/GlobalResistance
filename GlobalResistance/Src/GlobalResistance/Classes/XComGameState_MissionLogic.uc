//---------------------------------------------------------------------------------------
//  FILE:    XComGameState_MissionLogic.uc
//  AUTHOR:  James Rakich
//  PURPOSE: Interface for adding new mission logic to X-Com 2. Extend this class and then
...
function bool AnyPendingReinforcements()
{
  ...
  local XComGameState_AIReinforcementSpawner BaseAISpawnerState;
  local XComGameStateHistory History;
  ...
  foreach History.IterateByClassType(class'XComGameState_AIReinforcementSpawner', BaseAISPawnerState)
  {
    break;
  }

  // true if there are any active reinforcement spawners
  return (BaseAISPawnerState != None);
}
